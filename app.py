...
links_uteis = {
    ...
    "📊 Macro & Commodities": {
        "Investing Commodities": "https://br.investing.com/commodities",
    ...
    with st.spinner('Lendo indicadores globais...'):
        ...
        if not dados.empty:
            ...
            for nome, ticker in macros_tickers.items():
                try:
                    p = precos_fechamento[ticker].iloc[-1]
                    v_ant = precos_fechamento[ticker].iloc[-2]
                    var = ((p / v_ant) - 1) * 100
...
if btn_radar:
    with st.spinner('Sincronizando Mercado Real...'):
        ...
        data = buscar_dados_financeiros(ativos_todos)
